    ...
    def backtest(self, trade_mode=1, data_frequency='monthly'):
        ...
        if trade_mode == 1:
            ...
            with open(file_path, mode) as f:
                if mode == 'w':
                    f.write('Time,model_name,mean_return,sharpe_ratio,std_return,annualized_return,annualized_volatility,max_drawdown,win_rate\n')
                if self.count == 0:
                    f.write(f'{self.predictability_name}\n')
                f.write(f'{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())},{self.model_name},{metrics["mean_return"]:.4f},{metrics["sharpe_ratio"]:.4f},{metrics["std_return"]:.4f},{metrics["annualized_return"]:.4f},{metrics["annualized_volatility"]:.4f},{metrics["max_drawdown"]:.4f},{metrics["win_rate"]:.4f}\n')
